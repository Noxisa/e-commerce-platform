...
describe('Request routes', () => {
  ...
  test('POST /api/requests creates request when user exists', async () => {
    mockUserFind.mockResolvedValue({ id: 1, email: 'u@e.com' });
    mockRequestCreate.mockResolvedValue({ id: 99 });

    const token = jwt.sign({ userId: 1 }, process.env.JWT_SECRET);
    const res = await request(app).post('/api/requests').set('Authorization', `Bearer ${token}`).send({ furnitureType: 'chair', woodType: 'oak', dimensions: '10x10' });
    expect(res.status).toBe(201);
    expect(res.body.requestId).toBe(99);
  });

  test('POST /api/requests returns 404 when user not found', async () => {
    mockUserFind.mockResolvedValue(null);
    const token = jwt.sign({ userId: 2 }, process.env.JWT_SECRET);
    const res = await request(app).post('/api/requests').set('Authorization', `Bearer ${token}`).send({});
    expect(res.status).toBe(404);
  });

  test('GET /api/requests returns all for admin', async () => {
  ...
  test('GET /api/requests returns only user requests for non-admin', async () => {
  ...
  test('GET /api/requests/:id forbids access when not owner', async () => {
    mockUserFind.mockResolvedValue({ id: 1, isAdmin: false });
    mockRequestFindByPk.mockResolvedValue({ id: 5, UserId: 2 });
    const token = jwt.sign({ userId: 1 }, process.env.JWT_SECRET);
    const res = await request(app).get('/api/requests/5').set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(403);
  });

  test('PATCH /api/requests/:id/status invalid status returns 400', async () => {
    mockUserFind.mockResolvedValue({ id: 9, isAdmin: true });
    const token = jwt.sign({ userId: 9 }, process.env.JWT_SECRET);
    const res = await request(app).patch('/api/requests/1/status').set('Authorization', `Bearer ${token}`).send({ status: 'bad' });
    expect(res.status).toBe(400);
  });

  test('PATCH /api/requests/:id/status updates status when valid', async () => {
    mockUserFind.mockResolvedValue({ id: 9, isAdmin: true });
    const save = jest.fn().mockResolvedValue(true);
    mockRequestFindByPk.mockResolvedValue({ id: 1, status: 'pending', save });
    const token = jwt.sign({ userId: 9 }, process.env.JWT_SECRET);
    const res = await request(app).patch('/api/requests/1/status').set('Authorization', `Bearer ${token}`).send({ status: 'contacted' });
    expect(res.status).toBe(200);
    expect(save).toHaveBeenCalled();
  });

  test('DELETE /api/requests/:id deletes when admin', async () => {
    mockUserFind.mockResolvedValue({ id: 9, isAdmin: true });
    const destroy = jest.fn().mockResolvedValue(true);
    mockRequestFindByPk.mockResolvedValue({ id: 2, destroy });
    const token = jwt.sign({ userId: 9 }, process.env.JWT_SECRET);
    const res = await request(app).delete('/api/requests/2').set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(200);
