...
Express.js server running on port 5000 (configurable via PORT env)
CORS configured for localhost:3000
Auth middleware defined inline (lines 78-101) - JWT verification
Routes mounted: /api/auth and /api/orders
Database import issue: Line 5 imports ./config/database but actual file is ./database.js
File: server/database.js
PostgreSQL connection via Sequelize ORM
Connection pooling configured (max: 10, min: 0)
Auto-sync with {alter: true}
File: server/models/User.js
Fields: email (unique, validated), password (hashed with bcrypt via beforeCreate hook)
Missing: isAdmin field, isVerified field, verificationToken field, role field
File: server/models/Order.js
Fields: furnitureType, woodType, dimensions, notes, userEmail
Relation: belongsTo User
Missing: Will be renamed/repurposed to "Request" model with additional fields
File: server/routes/auth.js
POST /api/auth/signup - creates user, returns JWT token
Missing: POST /api/auth/login endpoint
Missing: Email verification system
Missing: Password comparison for login
File: server/routes/order.js (will become requests.js)
POST /api/orders - creates order with auth middleware
Uses middleware from ../middleware/auth but this directory doesn't exist
Issue: Auth middleware should be imported from server.js or separate middleware file
Frontend (Does Not Exist)
Directory: e-commerce-platform/user/ is completely empty
Desired End State
After implementation, the system will have:
Complete Frontend Application (React)
Home page with product grid, category filters, and search bar
Product detail pages with wood type selector, variants, and request form
Login/Register pages with email verification
User dashboard showing submitted requests
Admin panel for product and request management (protected by admin role)
Enhanced Backend API
Product CRUD endpoints (admin-only)
Request submission endpoint (authenticated users)
Login endpoint with email verification check
Email verification system
Admin middleware for role-based access
Request management endpoints (admin-only)
Database Models
Product model (with variants, wood types, pricing)
Request model (replacing Order)
Updated User model (with admin role and verification fields)
Verification Method
Users can browse products without authentication
Request submission requires authenticated and verified account
Admin panel accessible only at /admin route with admin role
Email notifications sent for: registration verification, request confirmations
---
...
From: const { connectDB } = require('./config/database');
To: const { connectDB } = require('./database');
...
role - DataTypes.STRING, defaultValue: 'customer', validate: isIn(['customer', 'admin'])
isVerified - DataTypes.BOOLEAN, defaultValue: false
verificationToken - DataTypes.STRING, allowNull: true
adminPassword - DataTypes.STRING, allowNull: true (only for admin users, separate from regular password)
Add new method after model definition:
comparePassword instance method that uses bcrypt.compare() to check password
Takes plaintext password as parameter
Returns Promise<boolean>
Admin Authentication Logic:
When user has role='admin', they must provide BOTH regular password AND adminPassword to access admin panel
Regular login uses regular password only
Admin panel login requires both passwords (two-step verification)
1.3 Create Product Model
Create new file: server/models/Product.js
Purpose: Store product catalog with variants and wood types
Fields:
name - DataTypes.STRING, allowNull: false (e.g., "Classic Wooden Chair")
category - DataTypes.STRING, allowNull: false, validate: isIn(['chair', 'table', 'cabinet', 'bed', 'shelf'])
description - DataTypes.TEXT, allowNull: false
basePrice - DataTypes.DECIMAL(10,2), allowNull: false (in PLN)
availableWoodTypes - DataTypes.ARRAY(DataTypes.STRING), defaultValue: [] (e.g., ['oak', 'pine', 'walnut'])
variants - DataTypes.JSONB, defaultValue: [] (array of objects with {name, priceModifier})
Example: [{name: "With Armrests", priceModifier: 200}, {name: "Cushioned Seat", priceModifier: 150}]
imageUrl - DataTypes.STRING, allowNull: true (future feature - leave null for now)
isActive - DataTypes.BOOLEAN, defaultValue: true (soft delete functionality)
...
Add: status - DataTypes.STRING, defaultValue: 'pending', validate: isIn(['pending', 'contacted', 'in_progress', 'completed', 'cancelled'])
...
File: server/routes/auth.js
Add POST /api/auth/login endpoint after signup (line 23):
Purpose: Authenticate existing users
Request body:
email (required, valid email format)
password (required, min 6 chars)
Implementation:
Validate input (use express-validator or manual checks)
Find user by email using User.findOne({ where: { email } })
If user not found: Return 401 with error: "Invalid email or password"
Check if user.isVerified === false: Return 403 with error: "Please verify your email before logging in"
Use user.comparePassword(password) to verify
If password incorrect: Return 401 with error: "Invalid email or password"
Generate JWT token with {userId: user.id, role: user.role}, expiresIn: '7d'
Return 200 with: {token, user: {id: user.id, email: user.email, role: user.role}}
...
Validate input
Find user by email
If user not found: Return 401 with error: "Invalid credentials"
Check if user.role !== 'admin': Return 403 with error: "Admin access required"
Verify regular password using comparePassword()
If regular password incorrect: Return 401 with error: "Invalid credentials"
Use bcrypt.compare(adminPassword, user.adminPassword) to verify admin password
If admin password incorrect: Return 401 with error: "Invalid admin credentials"
Generate JWT token with {userId: user.id, role: 'admin', isAdmin: true}, expiresIn: '7d'
Return 200 with: {token, user: {id: user.id, email: user.email, role: 'admin'}}
Add POST /api/auth/verify-email endpoint:
Purpose: Verify email with token from email link
Request body:
token (required, string)
Implementation:
Find user by verificationToken using User.findOne({ where: { verificationToken: token } })
If not found: Return 400 with error: "Invalid or expired verification token"
Update user: isVerified = true, verificationToken = null
Save user
Return 200 with: {message: "Email verified successfully"}
